...
class OrgListUtil {
    ...
    static async readAuthFiles(fileNames) {
        ...
        await fs.mkdir(core_1.Global.SFDX_DIR, { recursive: true });
        ...
        const allAuths = await Promise.all(fileNames.map(async (fileName) => {
            try {
                ...
                if (orgFileNames.includes(orgFileName)) {
                    const orgFileContent = JSON.parse(await fs.readFile((0, path_1.join)(core_1.Global.SFDX_DIR, orgFileName), 'utf8'));
