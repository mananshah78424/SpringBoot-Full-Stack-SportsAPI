...
const replacements_1 = require("../../src/convert/replacements");
...
class SourceComponent {
    ...
    get fullName() {
        ...
        if (this.parent && this.type.ignoreParentName) {
            if (!this.name) {
                throw new core_1.SfError(`Component was initialized without a name: ${this.xml} (${this.type.name})`);
            }
            return this.name;
        }
        else {
    ...
    get metaFilePath() {
        if (this.type.id === 'digitalexperience' && this.content && this.type.metaFileSuffix) {
            ...
            return (0, path_1.join)((0, path_1.dirname)(this.content), this.type.metaFileSuffix);
        }
        return this.xml;
    }
    get tree() {
    ...
    static createVirtualComponent(props, fs, forceIgnore) {
        ...
        throw new core_1.SfError(`Virtual Components must be constructed with a name: ${props.type.name}`);
    }
    walkContent() {
        const sources = [];
        if (this.content) {
    ...
    parseFromParentXml(parentXml) {
        ...
        const children = (0, kit_1.ensureArray)((0, ts_types_1.get)(parentXml, `${this.parent.type.name}.${this.type.xmlElementName ?? this.type.directoryName}`));
        ...
        if (!matched) {
            throw new core_1.SfError(`Unable to find matching parent xml file for ${this.xml}`);
        }
        return matched;
    }
    ...
    parse(contents) {
        ...
        const parsed = parser.parse(String(contents));
    ...
    getNonDecomposedChildren() {
        ...
        if (!this.type.children) {
            throw new core_1.SfError(`There are no child types for ${this.type.name}`);
        }
        for (const childTypeId of Object.keys(this.type.children.types)) {
            ...
            if (uniqueIdElement) {
                ...
                const childComponents = elements.map((element) => {
                    ...
                    if (!name) {
                        throw new core_1.SfError(`Missing ${uniqueIdElement} on ${childType.name} in ${this.xml}`);
                    }
