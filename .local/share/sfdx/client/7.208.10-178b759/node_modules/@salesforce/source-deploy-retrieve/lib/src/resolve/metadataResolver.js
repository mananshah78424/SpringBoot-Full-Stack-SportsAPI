...
class MetadataResolver {
    ...
    getSuggestionsForUnresolvedTypes(fsPath) {
        ...
        const closeMetaSuffix = new RegExp(/.+\.([^.-]+)(?:-.*)?\.xml/).exec((0, path_1.basename)(fsPath));
        let guesses;
        if (metaSuffix) {
        ...
        return guesses && guesses.length > 0
            ? [
                messages.getMessage('suggest_type_header', [(0, path_1.basename)(fsPath)]),
    ...
    parseAsMetadata(fsPath) {
        ...
        return ['DigitalExperience', 'ExperiencePropertyTypeBundle']
            ...
            .find((type) => fsPath.split(path_1.sep).includes(type.directoryName))?.name;
    }
    isMetadata(fsPath) {
