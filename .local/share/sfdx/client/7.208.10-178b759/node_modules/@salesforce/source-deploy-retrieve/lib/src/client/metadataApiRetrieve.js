...
class MetadataApiRetrieve extends metadataTransfer_1.MetadataTransfer {
    ...
    async post(result) {
        ...
        components ??= new collections_1.ComponentSet(undefined, this.options.registry);
    ...
    getPackageOptions() {
        ...
        return (packageOptions ?? []).map((po) => (0, ts_types_1.isString)(po) ? { name: po, outputDir: po } : { name: po.name, outputDir: po.outputDir ?? po.name });
    }
    async extract(zip) {
        const components = [];
