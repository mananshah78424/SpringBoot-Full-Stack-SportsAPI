...
class Telemetry extends kit_1.AsyncCreatable {
    ...
    static async acknowledgeDataCollection() {
        ...
        if (!Telemetry.cacheDir) {
            (0, debugger_1.debug)('Unable to check acknowledgment path because Telemetry.cacheDir is not set yet');
            return;
        }
        ...
        try {
            ...
            (0, debugger_1.debug)('Usage acknowledgement file already exists');
        }
        catch (error) {
            const err = error;
            if (err.code === 'ENOENT') {
                ...
                (0, debugger_1.debug)('Wrote usage acknowledgement file', acknowledgementFilePath);
            }
            else {
                (0, debugger_1.debug)('Could not access', acknowledgementFilePath, 'DUE TO:', err.code, err.message);
            }
        }
    }
    ...
    getCLIId() {
        ...
        try {
            ...
            (0, debugger_1.debug)('Unique CLI ID not found, generating and writing new ID to ', cliIdPath);
    ...
    record(data) {
        ...
        if (!dataToRecord.eventName) {
            ...
            (0, debugger_1.debug)('Missing event name!');
        }
        ...
        try {
            ...
            const error = err;
            (0, debugger_1.debug)(`Error saving telemetry line to file: ${error.message}`);
        }
    }
    ...
    async clear() {
        (0, debugger_1.debug)('Deleting the log file', this.getTelemetryFilePath());
        await fs.promises.unlink(this.getTelemetryFilePath());
    }
    async read() {
        try {
            (0, debugger_1.debug)(`Reading ${this.getTelemetryFilePath()}`);
            ...
            const err = error;
            (0, debugger_1.debug)(`Error reading: ${err.message}`);
    ...
    upload() {
        ...
        if (!telemetryDebug) {
            (0, debugger_1.debug)(`Spawning "${nodePath} ${processPath} ${Telemetry.cacheDir} ${this.getTelemetryFilePath()}"`);
            ...
            (0, debugger_1.debug)(`DEBUG MODE. Run the uploader manually with the following command:${os_1.EOL}${processPath} ${Telemetry.cacheDir} ${this.getTelemetryFilePath()}`);
        }
    }
    async init() {
        ...
        try {
            ...
            const err = error;
            if (err.code === 'ENOENT') {
                (0, debugger_1.debug)('Telemetry temp dir does not exist, creating...');
        ...
        (0, debugger_1.debug)(`Using telemetry logging file ${this.getTelemetryFilePath()}`);
    }
}
